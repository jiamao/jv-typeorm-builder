...
const {SapDriver} = require("typeorm/driver/sap/SapDriver");
const {RawSqlResultsToEntityTransformer} = require("typeorm/query-builder/transformer/RawSqlResultsToEntityTransformer");
const {SqlServerDriver} = require("typeorm/driver/sqlserver/SqlServerDriver");
const {PessimisticLockTransactionRequiredError} = require("typeorm/error/PessimisticLockTransactionRequiredError");
const {NoVersionOrUpdateDateColumnError} = require("typeorm/error/NoVersionOrUpdateDateColumnError");
const {OptimisticLockVersionMismatchError} = require("typeorm/error/OptimisticLockVersionMismatchError");
const {OptimisticLockCanNotBeUsedError} = require("typeorm/error/OptimisticLockCanNotBeUsedError");
const {JoinAttribute} = require("typeorm/query-builder/JoinAttribute");
const {RelationIdAttribute} = require("typeorm/query-builder/relation-id/RelationIdAttribute");
const {RelationCountAttribute} = require("typeorm/query-builder/relation-count/RelationCountAttribute");
const {RelationIdLoader} = require("typeorm/query-builder/relation-id/RelationIdLoader");
const {RelationIdMetadataToAttributeTransformer} = require("typeorm/query-builder/relation-id/RelationIdMetadataToAttributeTransformer");
const {RelationCountLoader} = require("typeorm/query-builder/relation-count/RelationCountLoader");
const {RelationCountMetadataToAttributeTransformer} = require("typeorm/query-builder/relation-count/RelationCountMetadataToAttributeTransformer");
const {QueryBuilder} = require("typeorm/query-builder/QueryBuilder");
const {LockNotSupportedOnGivenDriverError} = require("typeorm/error/LockNotSupportedOnGivenDriverError");
...
const {OracleDriver} = require("typeorm/driver/oracle/OracleDriver");
const {AbstractSqliteDriver} = require("typeorm/driver/sqlite-abstract/AbstractSqliteDriver");
const {OffsetWithoutLimitNotSupportedError} = require("typeorm/error/OffsetWithoutLimitNotSupportedError");
const {BroadcasterResult} = require("typeorm/subscriber/BroadcasterResult");
const {ObjectUtils} = require("typeorm/util/ObjectUtils");
const {DriverUtils} = require("typeorm/driver/DriverUtils");
const {AuroraDataApiDriver} = require("typeorm/driver/aurora-data-api/AuroraDataApiDriver");
const {CockroachDriver} = require("typeorm/driver/cockroachdb/CockroachDriver");
const {EntityNotFoundError} = require("typeorm/error/EntityNotFoundError");
const {TypeORMError} = require("typeorm/error/index");
/**
 * Allows to build complex sql queries in a fashion way and execute those queries.
 */
var SelectQueryBuilder = /** @class */ (function (_super) {
    _super = _super !== null && _super.QueryBuilder? _super.QueryBuilder: _super;
    ...
    SelectQueryBuilder.prototype.createSelectExpression = function () {
        ...
        var froms = this.expressionMap.aliases
            ...
            .map(function (alias) {
            if (alias.subQuery)
                return alias.subQuery + " " + alias.name;
            return _this.getTableName(alias.tablePath) + " " + alias.name;
        });
        var select = this.createSelectDistinctExpression();
        var selection = allSelects.map(function (select) { return select.selection + (select.aliasName ? " AS " + select.aliasName : ""); }).join(", ");
    ...
    SelectQueryBuilder.prototype.createJoinExpression = function () {
        ...
        var joins = this.expressionMap.joinAttributes.map(function (joinAttr) {
            ...
            if (!parentAlias || !relation) {
                ...
                return " " + joinAttr.direction + " JOIN " + destinationJoin + " " + destinationTableAlias +
            ...
            if (relation.isManyToOne || relation.isOneToOneOwner) {
                ...
                return " " + joinAttr.direction + " JOIN " + _this.getTableName(destinationTableName) + " " + destinationTableAlias + " ON " + _this.replacePropertyNames(condition + appendedCondition);
            }
                ...
                return " " + joinAttr.direction + " JOIN " + _this.getTableName(junctionTableName) + " " + junctionAlias_1 + " ON " + _this.replacePropertyNames(junctionCondition) +
                    " " + joinAttr.direction + " JOIN " + _this.getTableName(destinationTableName) + " " + destinationTableAlias + " ON " + _this.replacePropertyNames(destinationCondition + appendedCondition);
            }
        });
        return joins.join(" ");
    };
